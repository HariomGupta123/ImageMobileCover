...
const DesignConfigurator = ({
    ...
    const [options, setOptions] = useState<
        ...
        async function saveConfiguration(){
            try{
                const {left:leftcase,top:topcase,width,height}=phoneCaseRef.current!.getBoundingClientRect()
                const {left:containerLeft,top:containerTop}=containerCaseRef.current!.getBoundingClientRect()
                const leftOffset=leftcase - containerLeft
                const topOffset=topcase - containerTop
            ...
            function base64ToBlob(base64:string,mimeType:string){
                ...
                for (let i = 0; i< byteCharacters.length; i++) {  
                        ...
                        //charCodeAt() is used for unicode of specified character 
                            ...
                            <div className='flex flex-col gap-6'>
                                <RadioGroup value={options.color}
                                    ...
                                    <div className='mt-3 flex items-center space-x-3'>
                                        {COLORS.map((color) => (
                                            <RadioGroup.Option key={color.label} value={color} className={({ checked, active }) =>
                                                cn(
